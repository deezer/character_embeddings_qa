...
def get_context_data(tokens, quotes, corefs, fulltoks, idd, train_ids, window=50, use_minor_chars=False, unmask_quotes=False):
...
def main(readFolder, writeFolder=None, use_minor_chars=False, window_size=100, unmask_quotes=False, pass_if_no_candidates=True):#, max_quote_length=200):

    tokensFolder= "booknlpen/pdnc_output"
    quoteFolder=readFolder
    corefFolder = readFolder
    ...
    all_novels = [i for i in os.listdir(readFolder) if os.path.isdir(os.path.join(readFolder, i))]
    ...
    for novel in all_novels:
        ...
        novel_data = get_context_data(tokens, quotes, corefs, fulltoks, novel, None, window=window_size, use_minor_chars=use_minor_chars, unmask_quotes=unmask_quotes)#, max_quote_length=max_quote_length)
    ...
    if writeFolder is not None:
        for split, data in zip(['test'], [test_data]):
...
if __name__=='__main__':
    ...
    main(readFolder, writeFolder, use_minor_char, window_size=window_size, unmask_quotes=unmask_quotes, pass_if_no_candidates=pass_if_no_candidates)#, max_quote_length=max_quote_length)
