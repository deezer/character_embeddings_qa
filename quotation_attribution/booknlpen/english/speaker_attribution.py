...
class BERTSpeakerID(nn.Module):
    ...
    def __init__(self, base_model=None, mode="vanilla", unmask_quote=False):
        ...
        assert mode in ['vanilla', 'char_emb', 'char_and_luar_emb', "uar_scene", "uar_play", "semantics"], "arg: mode accepts values ['vanilla', 'char_emb', 'char_and_luar_emb', 'drama_luar']"
        super().__init__()
        ...
        if ("uar" in self.mode):#& ("drama" not in self.mode):
            style_dim = 512
        ...
        if not self.unmask_quote :
            if ("uar" in self.mode) or ("semantics" in self.mode): 
